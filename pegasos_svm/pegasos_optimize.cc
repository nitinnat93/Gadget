...
void LearnReturnLast(// Input variables
  ...
  // Initialize the file to store all the information from the algorithm.
  ofstream fs;
 ...
 startTime = get_runtime2();
  ...
  for (int i = 0; i < max_iter; ++i) {
    ...
    // Break out of loop if convergence occurs
          ...
          break;
        //std::cout <<"Algorithm converged at " 
            }
...
// Convergence condition
 ...
 if (max_iter > 1 && i%100 == 0){
          ...
          fs << i << ',' << trainTimePerIter <<"," << obj_value_prev << "," << obj_value << "," <<
            ...
            endTimePerIter = get_runtime2();
            notNeededTime += diff(startTimePerIter, endTimePerIter);
...
void ReadData(// input
  ...
  // update timeline
  readingTime = get_runtime2() - startTime;
